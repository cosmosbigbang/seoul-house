...
def run_home(total_df):
    ...
    selected_month = st.sidebar.radio(
        "확인하고 싶은 월을 선택하세요 👇", ["3월", "4월"]
    )
    month_dict = {"3월": 3, "4월": 4}
    st.markdown("<hr>", unsafe_allow_html=True)
    st.subheader(f"{sgg_nm} {selected_month} 아파트 가격 개요")
    st.markdown(
        "자치구와 월을 클릭하면 자동으로 각 지역구의 거래된 **최소가격**, **최대가격**을 확인할 수 있습니다."
    )
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("### 아파트 가격 상위 50")
    sorted_df = filtered_month[
        ["SGG_NM", "BJDONG_NM", "BLDG_NM", "BLDG_AREA", "OBJ_AMT"]
    ]
    st.dataframe(
        sorted_df.sort_values(by="OBJ_AMT", ascending=False)
        .head(50)
        .reset_index(drop=True)
    )
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("### 아파트 가격 하위 50")
    sorted_df = filtered_month[
        ["SGG_NM", "BJDONG_NM", "BLDG_NM", "BLDG_AREA", "OBJ_AMT"]
    ]
    st.dataframe(
        sorted_df.sort_values(by="OBJ_AMT", ascending=True)
        .head(50)
        .reset_index(drop=True)
    )
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.caption(
        "출처 : [서울시 부동산 실거래가 정보](https://data.seoul.go.kr/dataList/OA-21275/S/1/datasetView.do)"
    )
